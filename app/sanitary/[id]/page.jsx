...
const page = ({params}) => {
    ...
    const fetchSanitaryData = async () => {
        try {
            ...
            if (response.data && response.data.success) {
                setData(response.data.sanitary);
            } else {
                setError(response.data?.message || 'Sanitary item not found');
            }
        } catch (error) {
            ...
            if (error.response) {
                ...
                const message = error.response.data?.message || 'Failed to load sanitary details';
                ...
                setError('Failed to load sanitary details');
            }
        } finally {
            setIsLoading(false);
        }
        ...
        return (
            <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 flex items-center justify-center">
                <div className="text-center">
                    ...
                    <h3 className="text-xl font-semibold text-gray-800 mb-2">Loading Sanitary Details</h3>
        ...
        return (
            <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 flex items-center justify-center">
                <div className="text-center max-w-md mx-auto px-6">
                    ...
                    <h2 className="text-3xl font-bold text-gray-800 mb-4">Sanitary Item Not Found</h2>
                    <p className="text-gray-600 mb-8 text-lg">The sanitary item you're looking for doesn't exist or has been removed.</p>
                    <Link href="/" className="inline-block bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-8 rounded-xl transition-all duration-300 transform hover:scale-105 shadow-lg">
                        Browse Sanitary Items
                    </Link>
                </div>
            </div>
        )
    ...
    // Format category and material for display
    ...
    const discountPercentage = data.originalPrice && data.price < data.originalPrice 
        ...
        <section className="relative">
            <div className="relative flex items-center justify-center bg-gray-100 py-4 sm:py-6 md:py-8 px-4 sm:px-6">
                <div className="relative w-full max-w-[1200px] h-[300px] sm:h-[400px] md:h-[500px] lg:h-[700px] overflow-hidden rounded-xl sm:rounded-2xl shadow-2xl">
                    <Image 
                        src={normalizeImageUrl(data.image || '/placeholder-sanitary.jpg')} 
                        alt={data.name} 
                        fill
                    ...
                    <div className="absolute bottom-3 left-3 right-3 sm:bottom-4 sm:left-4 sm:right-4 md:bottom-6 md:left-6 md:right-6 text-white">
                        ...
                        <div className="flex items-center text-sm sm:text-base md:text-lg lg:text-xl">
                            ...
                            <span className="drop-shadow-lg line-clamp-1">{formatText(data.category)} • {data.brand}</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl p-4 sm:p-6 md:p-8">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-4 sm:mb-6 md:mb-8">
                            ...
                            <div className="text-center p-3 sm:p-4 bg-gray-50 rounded-lg sm:rounded-xl">
                                ...
                                <p className="text-xs sm:text-sm text-gray-600">{formatText(data.material)}</p>
                            </div>
                            ...
                            <div className="text-center p-3 sm:p-4 bg-gray-50 rounded-lg sm:rounded-xl">
                                ...
                                <h3 className="font-semibold text-gray-800 text-xs sm:text-sm">Brand</h3>
                                <p className="text-xs sm:text-sm text-gray-600">{data.brand || 'N/A'}</p>
                            </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                            ...
                            <div className="bg-gray-50 rounded-lg p-4">
                                <h4 className="text-sm font-semibold text-gray-600 mb-1">Brand</h4>
                                <p className="text-base font-medium text-gray-800">{data.brand || 'N/A'}</p>
                            </div>
                        </div>
                    ...
                    <div className="bg-linear-to-r from-red-600 to-red-700 rounded-xl sm:rounded-2xl shadow-xl p-6 sm:p-8 text-white">
                        <div className="flex flex-col md:flex-row items-center justify-between">
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto">
                                ...
                                <a 
                                    href={`https://wa.me/${contactPhone.replace(/\D/g,'')}?text=${encodeURIComponent(`Hello, I'm interested in ${data.name} (${formatText(data.category)}, ${data.brand}). Is it available?`)}`}
                                    target="_blank" 
                            ...
                            <div className="space-y-4">
                                {data.originalPrice && data.originalPrice > data.price && (
                                    <div className="flex justify-between items-center py-3 border-b border-gray-100">
                                        <span className="text-gray-600 font-medium text-sm sm:text-base">Original Price</span>
                                ...
                                {discountPercentage && (
                                    <div className="flex justify-between items-center py-3 bg-green-50 rounded-xl px-4">
                                        <span className="text-green-700 font-semibold text-sm sm:text-base">You Save</span>
                                        <span className="text-xl sm:text-2xl font-bold text-green-600">
                                            {(data.originalPrice - data.price)?.toLocaleString()}৳ ({discountPercentage}%)
                                        </span>
                                    </div>
                                )}
                                ...
                                <div className="flex justify-between items-center py-3 border-b border-gray-100">
                                    <span className="text-gray-600 font-medium text-sm sm:text-base">Material</span>
                                    <span className="text-lg sm:text-xl text-gray-800 font-semibold">
                                        {formatText(data.material)}
                                    </span>
                                </div>
                                
                                <div className="flex justify-between items-center py-3 border-b border-gray-100">
                                    <span className="text-gray-600 font-medium text-sm sm:text-base">Brand</span>
                                    <span className="text-lg sm:text-xl text-gray-800 font-semibold">
                                        {data.brand || 'N/A'}
                                    </span>
                                </div>
                        ...
                        <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl p-4 sm:p-6">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="font-semibold text-gray-800 text-sm sm:text-base">{data.brand || 'N/A'}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-600 text-sm sm:text-base">Material</span>
                                    <span className="font-semibold text-gray-800 text-sm sm:text-base">{formatText(data.material)}</span>
                                </div>
                                <div className="flex justify-between items-center">
