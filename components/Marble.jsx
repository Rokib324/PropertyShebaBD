...
const MarvelSanitary = () => {
  ...
  // Fallback data if no marbles are available
  ...
  // Convert marble data to display format
  const allProperties = marbles.length > 0 ? marbles.map((marble, index) => ({
    id: marble._id || index,
    title: marble.name,
    location: marble.origin,
    originalPrice: marble.price,
    discountedPrice: marble.pricePerSqft,
    discount: Math.round(((marble.price - marble.pricePerSqft) / marble.price) * 100) || 10,
    image: marble.image,
    type: marble.category,
    description: marble.description,
    color: marble.color,
    finish: marble.finish,
    thickness: marble.thickness,
    size: marble.size,
    stock: marble.stock,
